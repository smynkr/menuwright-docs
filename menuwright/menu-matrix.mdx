...
To fill the gaps, enter costs manually per item, upload a cost file (see [CSV import](/menuwright/csv-import)), or use **AI cost estimation** (Core and Pro plans) on the Data page to estimate missing ingredient costs from each item's name, category, and menu price. Estimated costs are marked with an `est.` chip and can be overridden by a manual edit anytime.
