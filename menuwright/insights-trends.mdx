...
description: "Use Insights for bundle, cannibalization, and forecast views, then use Trends to compare your analysis history."
keywords: ["insights", "trends", "bundles", "cannibalization", "forecast", "analysis history"]
---
...
- **Revenue Trend** — the weekly growth rate when a trend is available. If there is not enough data, the card shows `--` with a **Run an analysis** link that takes you to onboarding.
...
Each forecast chart includes a collapsible data table — **View the revenue forecast data table**, **View the day-of-week data table**, or **View the monthly data table** — that restates the charted values as text, so the numbers remain readable with a screen reader or when you want exact figures instead of reading them off the chart.
...
With no analyses, Trends asks you to run your first analysis, notes that menu data is the missing input for this view, and links back to onboarding.
...
- **Analysis History** — the analysis date, analyzed period, item count, total revenue, average contribution margin (shown as **Not available** when an analysis has no margin data), and status.
