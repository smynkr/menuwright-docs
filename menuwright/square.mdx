...
| Capability | Detail |
|---|---|
| **Catalog sync** | One menu item per priced variation (parent name + variation name), categories, and prices, mapped into MenuWright's item model |
| **Sales sync** | Itemized order history using realized revenue (charged amount net of tax, discount-aware), aggregated into daily sales records for the analysis window |
...
| Item | Detail |
|---|---|
| **Menu items** | Fetched from the Square Catalog API, one row per priced variation keyed by variation ID; categories resolved via the catalog's category objects |
| **Sales** | Fetched from the Square Orders API using realized revenue (charged amount net of tax) and normalized into daily `SalesRecord` rows |
...
<Note>
  Square order line items reference variation IDs, not parent item IDs. MenuWright imports one menu item per priced variation so sales join correctly — for example, an item with "Regular" and "Large" variations appears as two rows: "Latte (Regular)" and "Latte (Large)". Free items (price $0) are imported as legitimate menu rows, not skipped.
</Note>
