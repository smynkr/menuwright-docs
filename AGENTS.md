# MenuWright Docs agent guide

## Memory routing

1. Read `.codex/harness-memory.json`; the project is `menuwright-docs` and
   the only permitted Graphiti group is `group_id=menuwright-docs`.
...
Flat MDX in `menuwright/` plus `docs.json` are canonical; `content/docs/` is
...
`menuwright/**` route prefix in canonical content and the rewrite list in
sync.
